...
def create_llm_from_config(llm_name, model_config, hparams):
    if 'gpt' in llm_name:
        llm = GPT(
...
class HuggingFaceLLM(LanguageModelBase):
    ...
    def __init__(self, model_config, hparams, device='cuda:0'):

        super(HuggingFaceLLM, self).__init__(model_config, hparams)
    ...
    def __call__(
            self, batch, temperature=0.0, max_num_tokens=100, top_p=1.0, json_format=False, stops=None):
...
class APILanguageModel(LanguageModelBase):
    ...
    def __init__(self, model_config, hparams):
        super(APILanguageModel, self).__init__(model_config, hparams)

    def __call__(self, batch, temperature=0.0, max_num_tokens=100, top_p=1.0, json_format=False, stops=None):
            ...
            for _ in range(self.API_MAX_RETRY):
                try:
                    response = self.query_openai_api(
                        conv, temperature, max_num_tokens, top_p, json_format=json_format, stops=stops
                    )
    ...
    def query_openai_api(self, conv, temperature, max_num_tokens, top_p, seed=None, json_format=False, stops=None):
        ...
        if json_format and (self.model_config['model_name'] in ['gpt-3.5-turbo-1106']):
...
class GPT(APILanguageModel):
    ...
    def __init__(self, model_config, hparams):
        ...
        super(GPT, self).__init__(model_config, hparams)
...
class VirtualLLM(APILanguageModel):
    ...
    def __init__(self, model_config, hparams):
        ...
        super(VirtualLLM, self).__init__(model_config, hparams)
        self.client = openai.Client(
            api_key="EMPTY",
            base_url=hparams['VIRTUAL_LLM_URL'],
        )
    ...
    def query_openai_api(
            self, conv, temperature, max_num_tokens, top_p, seed=None, json_format=False, stops=None):
        #! Use continuation api for VirtualLLM model instead of ChatAPI
